...
from model.hed import create_model
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(tqdm(train_loader, desc='[Train] Epoch' + str(epoch + 1) + '/' + str(
            ...
            loss = compute_loss(outputs, labels, args)  # 计算损失
            metrics = compute_metrics(outputs, labels, args)  # 计算评价指标
            loss.backward()  # 反向传播
            optimizer.step()  # 更新参数
        ...
        if (epoch + 1) % args.val_every == 0:
            model_name = 'HED_epoch_{}_dice_{:.6f}.pth'.format(epoch + 1, m_val_dice)
